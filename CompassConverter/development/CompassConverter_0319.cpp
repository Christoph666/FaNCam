	...
	for (int iMeasurement=0; iMeasurement<1; iMeasurement++) {
		...
		for (int iDEV=0; iDEV<2; iDEV++) {
			for (int iCHN=0; iCHN<8; iCHN++) {
				...
				string filename = to_string(iCHN) + (string)"@N6730B #3-11-"+board_ID[iDEV]+(string)"_Data_"+measurement+(string)".bin";
				...
				if ((is.good())) {
					...
					int bytes_per_block = 22+nSamples_0*2;
					...
					while(1) {
						...
						for (int iBlock=0; iBlock<blocks_per_buffer; iBlock++) {
							...
							iPow=0;
							for (int i=16+iStart; i<18+iStart; i++) {
								flags += (unsigned short)(unsigned char)buffer[i] * pow(2,iPow*8);
								iPow++;
							}
							iPow=0;
							for (int i=18+iStart; i<22+iStart; i++) {
							...
							for (int j=1; j<nSamples_0; j++) {
								iPow=0;
								sample=0;
								for (int i=22+iStart+2*j; i<24+iStart+2*j; i++) {
